...
class TestFormatStatusToggles:
    ...
    @pytest.mark.parametrize("toggle", [
        "context_pct", "max_tokens", "server", "vram", "fit",
        "message_count", "temperature", "top_p", "tok_per_s",
    ])
    def test_basic_toggle_produces_output(self, toggle):
        result = format_status(self.SNAP, {toggle}, "short")
        assert result != ""
